...
function usersTableColumns(userId: string | undefined) {
  return [
    {
      id: "name",
      accessorKey: "name",
                ...
                {userId !== row.original.id && (
                  <>
                    ...
                    <UpdateUserActivationDialog
                      id={row.original.id}
                      toActive={!row.original.isActive}
                    >
                      <DropdownMenuItem closeOnClick={false}>
...
export function UsersTable({ data, isLoading }: { data: GetUsers["items"]; isLoading: boolean }) {
  ...
  return (
    <DataTable
      columns={usersTableColumns(session.data?.user?.id)}
      data={data}
      isLoading={isLoading}
    >
      {(table) => (
